...
class TestBarkiPoprawnosciowe(unittest.TestCase):
    def setUp(self):
        barki.AKTYWNY = False
        barki.counter = 0
    ...
    def test_logika_zliczania_powtorzen(self, mock_imencode, mock_ui, mock_kat, mock_pobierz, mock_cam,
        ...
        mock_inicjalizuj.return_value = (0, "down", "", "", "")
        mock_cam.return_value = (MagicMock(), MagicMock(), False)

        mock_active_results = MagicMock()
        mock_active_results.pose_landmarks.landmark = "dowolne_dane"
        ...
        self.assertEqual(barki.counter, 1)
